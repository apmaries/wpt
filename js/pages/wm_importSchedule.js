...
const importStorage = "wmImpSchData";
const ignoredStorage = "wmImpSchIgnored";
...
function enableButtons() {
  // get bu data complete, enable buttons on page
...
function openLocalCSV(file, buData, callback) {
  ...
  reader.onload = function (event) {
    ...
    const expectedHeaders =
      "BU Name,Agent Email,Activity Code Name,Start,End,Description,Is Paid";
...
// get time zone offset for selected BU
const dropdown = document.getElementById("bu-selector");
const tzDropdown = document.getElementById("tz-selector");
const buTzItem = document.getElementById("bu-tz-item");
const objects = JSON.parse(sessionStorage.getItem(wfmBuStorage));

// Event listener to handle dropdown selection
dropdown.addEventListener("change", function () {
  const selectedName = this.value;

  // Find the object based on the selected name
  const selectedObject = objects.find((object) => object.name === selectedName);

  // Store the offset in the 'offset' variable
  if (selectedObject) {
    const tzId = selectedObject.timeZone;
    const offset = selectedObject.offset;
    console.log(`Selected offset: ${offset}`);

    // Update the 'bu-tz-item' element's innerHTML and value
    buTzItem.innerHTML = `Business Unit (${tzId})`;
    buTzItem.value = offset;
  }
});
